...
window.reprintReceipt = function(purchaseId, storeId) {
  ...
  const storeLabel    = getStoreLabel(purchase._storeId) || '';
  ...
  const verifiedRows = purchase.verified ? `
    ...
    <div class="info-row"><span class="info-label">Verified By</span><span class="info-value">${cashierName}</span></div>` : '';

  const printWindow = window.open('', '', 'width=800,height=900');
  printWindow.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">
<title>CGrocs Receipt \u2014 ${purchase.id}</title>
<style>
...
.info-row:last-child{border-bottom:none;}.info-label{color:#6b7280;}.info-value{font-weight:500;text-align:right;}
...
<div class="info-block">
  ...
  <div class="info-row"><span class="info-label">Purchase ID</span><span class="info-value mono">${purchase.id}</span></div>
  ...
  <div class="info-row"><span class="info-label">Reference</span><span class="info-value mono">${purchase.reference || purchase.id || '\u2014'}</span></div>
  ${verifiedRows}
</div>
