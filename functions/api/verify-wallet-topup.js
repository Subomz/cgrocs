...
//
//  Flow:
//    1. Verify the Paystack reference server-side
//    2. Confirm the amount matches what the client claimed
//    3. Check the reference hasn't been used before (replay guard)
//    4. Atomically:
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (!reference || typeof reference !== 'string') {
      return Response.json({ error: 'Missing or invalid reference' }, { status: 400, headers: corsHeaders });
    }
    if (!uid || typeof uid !== 'string') {
      return Response.json({ error: 'Missing or invalid uid' }, { status: 400, headers: corsHeaders });
    }
    ...
    if (psData.data.amount !== expectedKobo) {
      console.error(
        `Amount mismatch: expected ${expectedKobo} kobo, got ${psData.data.amount} kobo`
      );
      return Response.json({ error: 'Amount mismatch' }, { status: 400, headers: corsHeaders });
    }
    ...
    return Response.json({ success: true, newBalance }, { headers: corsHeaders });
