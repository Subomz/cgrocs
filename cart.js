...
// Returns the service charge (in naira) to add on top of the cart total (Paystack)
...
// Returns the convenience fee for wallet payments (lower than Paystack)
...
// Returns the active service charge based on current payment method
function getActiveServiceCharge(cartTotal) {
  return window._walletPaymentActive
    ? getWalletServiceCharge(cartTotal)
    : getServiceCharge(cartTotal);
}
...
function updateCartDisplay() {
  ...
  if (cartTotal) {
    const charge = cart.length > 0 ? getActiveServiceCharge(grandTotal) : 0;
