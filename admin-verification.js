...
window.printReceipt = async function(purchaseId) {
  let purchase = null;
  try {
    // Use a targeted query (1 read) instead of scanning the whole collection
    const q = query(col('purchases'), where('id', '==', purchaseId), limit(1));
  ...
  printWindow.document.write(`<!DOCTYPE html><html><head><title>Receipt - ${escapeHtml(purchase.id)}</title>
    ...
    <div class="rh">
      <h1>CGrocs</h1>
      <h2>Purchase Receipt${storeLabel ? ` · ${escapeHtml(storeLabel)}` : ''}</h2>
      ${purchase.verified
        ? '<div class="badge v-badge">✓ VERIFIED</div>'
        : '<div class="badge p-badge">⏳ PENDING</div>'}
    </div>
    <div class="sec"><div class="sec-t">Customer</div>
      ...
      <div class="dr"><span>Email</span><span>${escapeHtml(customerEmail)}</span></div>
    </div>
    <div class="sec"><div class="sec-t">Order Info</div>
      ...
      ${purchase.verified ? `
      <div class="dr"><span>Verified At</span><span>${new Date(purchase.verifiedDate).toLocaleString()}</span></div>
      <div class="dr"><span>Verified By</span><span>${escapeHtml(cashierName)}</span></div>` : ''}
    </div>
    ...
    <div class="footer">
      <p>Thank you for shopping with CGrocs!</p>
      <p style="margin-top:4px">Printed at ${new Date().toLocaleString()}</p>
    </div>
